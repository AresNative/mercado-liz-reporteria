...
export default function Report() {
    ...
    const fetchSuggestions = useCallback(async () => {
        ...
        try {
            ...
            const response: ApiResponse = await safeCall(() => getData({
                ...
                filtros: {
                    Agregaciones: [{
                        Key: searchColumn.tableField === "art.Descripcion1" ? "art.Descripcion1" : searchColumn.tableField,
                        Alias: "Descripcion1",
                        Operation: "DISTINCT"
                    }],
                    filtros: [{
            ...
            // Verificar si la petición fue abortada
            if (controller.signal.aborted) {
                console.log("Petición de sugerencias abortada intencionalmente");
                return;
            }
            ...
            if (response.error) {
                ...
                if (response.error.name === 'AbortError' ||
                    response.error.message?.includes('aborted') ||
                    response.error.code === 'ERR_CANCELLED') {
                    console.log("Petición de sugerencias cancelada");
                    return;
                }
            ...
            if (error.name === 'AbortError' ||
                error.message?.includes('aborted') ||
                error.code === 'ERR_CANCELLED') {
                console.log("Petición de sugerencias cancelada (catch)");
                return;
            }
            ...
            if (!suggestionsAbortControllerRef.current?.signal.aborted) {
                ...
                console.log("Petición de sugerencias abortada, limpiando sin estado");
            }
        }
    ...
    const handleSearchColumnChange = (column: SearchColumn) => {
        ...
        // Cancelar peticiones de sugerencias pendientes
        if (suggestionsAbortControllerRef.current) {
            suggestionsAbortControllerRef.current.abort();
        }
