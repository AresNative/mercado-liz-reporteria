...
// Los modales son pesados (fetches, gráficas, tablas propias) y no se
// necesitan hasta que el usuario los abre: se cargan bajo demanda para
// no inflar el bundle inicial de la página.
...
// ─── Configuración de reportes, tipos y helpers ────────────────────────────────
// Viven en ./utils/report-utils.ts (compartidos con modal-reporting.tsx),
// así se mantiene una sola fuente de verdad en vez de dos copias que
// pueden desincronizarse.
export default function Analisis() {
    const [manager] = useManagmentRead();

    // Cada modal controla su propio "¿ya se abrió alguna vez?" + el dispatch
    // que lo marca como abierto en redux (para animación/visibilidad de <Modal>).
    ...
    return (
        <>
            ...
            {reportingModal.mounted && (
                <ModalReporting reportType={selectedReport} />
            )}
            <Footer />
        </>
    );
