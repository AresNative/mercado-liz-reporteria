...
const accessSlice = createSlice({
  ...
  reducers: {
    checkIn: (
      ...
      const existingRecord = state.currentlyInGym.find((r) => r.memberId === action.payload.memberId)
      if (existingRecord) {
    ...
    checkOut: (state, action: PayloadAction<string>) => {
      const recordIndex = state.records.findIndex((r) => r.memberId === action.payload && !r.checkOutTime)
      const currentIndex = state.currentlyInGym.findIndex((r) => r.memberId === action.payload)

      if (recordIndex !== -1) {
        const checkOutTime = new Date().toISOString()
        const checkInTime = new Date(state.records[recordIndex].checkInTime)
        const duration = Math.floor((new Date(checkOutTime).getTime() - checkInTime.getTime()) / (1000 * 60))
    ...
    updateDailyStats: (state) => {
      ...
      const peakHour =
        Object.entries(hourCounts).reduce((a, b) => (hourCounts[a[0]] > hourCounts[b[0]] ? a : b))?.[0] || "00:00"
...
export const { checkIn, checkOut, setLoading, clearError, updateDailyStats } = accessSlice.actions
